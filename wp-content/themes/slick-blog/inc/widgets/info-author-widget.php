<?php
if ( ! class_exists( 'Slick_Blog_Author_Info_Widget' ) ) {
	/**
	 * Adds Slick_Blog_Author_Info_Widget Widget.
	 */
	class Slick_Blog_Author_Info_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$slick_blog_author_info_widget_ops = array(
				'classname'   => 'blog-author-section author-style-1',
				'description' => __( 'Retrive Author Information Widget', 'slick-blog' ),
			);
			parent::__construct(
				'slick_blog_author_info_widget',
				__( 'Ascendoor Author Info Widget', 'slick-blog' ),
				$slick_blog_author_info_widget_ops
			);
		}
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'author_image_url' ) ); ?>"><?php esc_html_e( 'Author Image URL', 'slick-blog' ); ?></label>:<br />
				...
				<input type="button" class="select-img" value="<?php esc_attr_e( 'Upload', 'slick-blog' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'slick-blog' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'slick-blog' ); ?>" style="margin-top:5px;" /><br/>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e( 'Author Name:', 'slick-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'address' ) ); ?>"><?php esc_html_e( 'Author Address:', 'slick-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'description' ) ); ?>"><?php esc_html_e( 'Description :', 'slick-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'open_link_new_tab' ) ); ?>"><?php esc_html_e( 'Open Social link in New Tab', 'slick-blog' ); ?>:</label>
				<input type="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'open_link_new_tab' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'open_link_new_tab' ), 'slick-blog' ); ?>"  <?php checked( $open_link_new_tab, true ); ?> />
			</p>
			<?php
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'link' . '-' . $i ) ); ?>"><?php sprintf( esc_html__( 'Link %s :', 'slick-blog' ), $i ); ?></label>
		...
		public function update( $new_instance, $old_instance ) {
			...
			$instance['open_link_new_tab'] = slick_blog_sanitize_checkbox( $new_instance['open_link_new_tab'] );
			for ( $i = 1; $i <= 4; $i++ ) {
 ...
 *
 * @since Slick Blog 1.0
 **/
function slick_blog_author_info_widget_image_upload_enqueue( $hook ) {
	...
	wp_enqueue_media();
	wp_enqueue_script( 'author-info-widget-image-upload-script', get_template_directory_uri() . '/assets/js/upload.min.js', array( 'jquery' ), SLICK_BLOG_VERSION, true );
}

add_action( 'admin_enqueue_scripts', 'slick_blog_author_info_widget_image_upload_enqueue' );
